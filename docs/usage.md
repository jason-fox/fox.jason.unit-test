...
the plugin to be able to generate coverage information. A copy of each `*.xsl` file is also saved with the `*.orig`
suffix.
...
`coverage.xml` file will be created along with the test results and a coverage report. This can be forwared to Coveralls
using the standard maven plug-in as shown:
